...
from sklearn.metrics import (balanced_accuracy_score, f1_score, precision_score,
                             ...
                             recall_score, precision_score)  # imported once at top
...
def train(model_name: str, num_leaves: int, learning_rate: float, n_estimators: int,
          max_depth: int, colsample_bytree: float, subsample: float, min_child_samples: int,
          reg_alpha: float, reg_lambda: float, min_split_gain: float, feature_fraction: float,
          bagging_fraction: float, bagging_freq: int, metric: str,
    ...
    # Threshold metrics plot (no local reimport here)
    ...
    if os.path.exists(incumbent_path) and os.path.exists(incumbent_meta_path):
        ...
        if bal_acc > incumbent_bal_acc:
            print("New LightGBM model is better. Replacing incumbent.")
            replace_flag = True
        else:
            print("New LightGBM model is not better. Saving candidate separately.")
    else:
        print("No incumbent LightGBM model found. Saving new model as incumbent.")
        replace_flag = True
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model-name", default="LightGBM")
    ...
    train(args.model_name, args.num_leaves, args.learning_rate, args.n_estimators, args.max_depth,
          args.colsample_bytree, args.subsample, args.min_child_samples, args.reg_alpha, args.reg_lambda,
          args.min_split_gain, args.feature_fraction, args.bagging_fraction, args.bagging_freq, args.metric,
