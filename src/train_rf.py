...
def train(model_name: str, n_estimators: int, max_depth: int, min_samples_split: int,
          min_samples_leaf: int, max_features: str, bootstrap: bool,
          min_weight_fraction_leaf: float, max_leaf_nodes: int, criterion: str,
    ...
    # --- Additional: Compute metrics across thresholds ---
    ...
    # Create a plot for metrics vs threshold
    ...
    # Log the threshold plot as an image
    thresh_img = wandb.Image(thresh_fig_path, caption="Metrics vs Threshold")

    # Log only numeric metrics and images to wandb
    wandb.log({
    ...
    if os.path.exists(incumbent_path) and os.path.exists(incumbent_meta_path):
        ...
        if bal_acc > incumbent_bal_acc:
            print("New RandomForest model is better. Replacing incumbent.")
            replace_flag = True
        else:
            print("New RandomForest model is not better. Saving candidate separately.")
    else:
        print("No incumbent RandomForest model found. Saving new model as incumbent.")
        replace_flag = True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RandomForest Trainer")
    ...
    parser.add_argument("-f", "--max-features", type=str, default='log2')
    parser.add_argument("-b", "--bootstrap", type=bool, default=True)
    parser.add_argument("-w", "--min-weight-fraction-leaf", type=float, default=0.0)
    parser.add_argument("-x", "--max-leaf-nodes", type=int, default=None)
    parser.add_argument("-c", "--criterion", type=str, default="entropy")
    ...
    train(args.model_name, args.n_estimators, args.max_depth, args.min_samples_split, args.min_samples_leaf,
          args.max_features, args.bootstrap, args.min_weight_fraction_leaf, args.max_leaf_nodes, args.criterion,
